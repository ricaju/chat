      ...
      <div key={comment.id}>
        <div className={style.date}>
          {checkDateDifference(comment.timestamp)
            ? dateConverter(comment.timestamp, "dddd, D. MM. YYYY.")
            : "Today"}
        </div>
...
function useChat() {
  ...
  const groupedByReplays = groupBy(
    comments.filter((comment: Comment) => comment.parent_id),
    "parent_id"
  );
  const keys = Object.keys(groupedByReplays);

  const commentsWithReplays = comments.reduce((withReplays: Comment[], comment: Comment) => {
